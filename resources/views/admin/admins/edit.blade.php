...
@section('content')
@include('admin.admins.partials.header' ,['title'=>__('Edit User')])
...
<div class="form-group{{$errors->has('name') ? 'has-danger' :''}}">
    ...
    <input type="text" name="name" id="input-name"class="form-control form-control-alternative{{$errors->has('name') ? 'is-invalid' :''}}" placeholder="{{__('Name')}}" value="{{$admin->name}}" required autofocus>
...
<input type="email" name="email" id="iput-email" class="form-control form-control-alternative{{$errors->has('email') ? 'is-invaild' :''}}" placeholder="{{__('Email')}}" value="{{$admin->email}}" required>
